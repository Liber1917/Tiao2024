...
void ADC_Configuration(void)
{
    ...
    // 设置 TIM3 为外置触发源
    ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T3_TRGO;					//扫描模式
...
void DMA1_Channel1_IRQHandler(void)
{
	if (DMA_GetITStatus(DMA1_IT_TC1) != RESET) //检查DMA1通道1传输结束标志
	{
		...
		CodeValue = DAM_ADC_Value[0];
		Voltage = (float)VREF * DAM_ADC_Value[0] / 4095 / 1000;

		PRINTF_LOG("Code Value = %ld , Voltage = %2.4f\n", CodeValue,Voltage); //输出ADC值
	}
}
